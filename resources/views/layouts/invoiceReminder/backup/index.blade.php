    ...
    @endif --}}
                        <div class="panel panel-default">
                            <div class="panel-body">
                                ...
                                <a href="{{ route('invoiceReminder.create') }}"
                                    class="btn btn-primary btn-xs pull-right py-0">Create New Reminder</a>
                                {{-- {{$now}} --}} <div class="table-responsive">
                                    <table class="table table-responsive table-bordered table-stripped"
                                        id="tableInvoiceReminder" style="margin-top:10px;">
                                        <thead>
                                            <tr>
                                                <th>#</th>
                                        ...
                                        <tbody>
                                            @foreach ($invoice_reminders as $invoice_reminder)
                                                <tr>
                                                    ...
                                                    <td>{{ $invoice_reminder->currency }}
                                                        {{ number_format($invoice_reminder->net_value, 2, '.', ',') }}</td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        ...
                                                        <a href="{{ route('invoiceReminder.edit', $invoice_reminder->ir_id) }}"
                                                            class="btn btn-sm btn-primary btn-xs py-0"><i
                                                                class="bi bi-pencil"></i></a>
                                                        <form
                                                            action="{{ route('invoiceReminder.destroy', $invoice_reminder->ir_id) }}"
                                                            method="post">
                                                            @csrf
                                                            @method('delete')
                                                            <button type="submit"
                                                                class="btn btn-sm btn-danger btn-xs py-0 show_confirm"><i
